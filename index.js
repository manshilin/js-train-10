 ...
 * Крім того, вона перетворює значення числових властивостей на рядки і замінює пробіли на 
 * //підкреслення.
...
/**
 * Функція `customEntries` повертає масив, що містить усі властивості об'єкта та їх значення у 
 * вигляді масивів.
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  let freezeObj = Object.freeze(assignObj);
...
/**
 * Функція keysAndValues приймає об'єкт, виводить в консоль його ключі та значення, і повертає
 *  масив з масивами ключів та значень.
...
function checkOwnership(obj1, obj2, key) {
  if(typeof obj1 !== 'object' || typeof obj2 !== 'object' || typeof key !== 'string') {
    return false;
  }
...
console.log(checkOwnership({ a: 1, b: 2, c: 3 }, { a: 1, b: 2, c: 4 }, "b")); // true

// Завдання: 11

/**
 * Функція getObjectValuesSum приймає об'єкт, в якому значення є числами, і повертає суму всіх
 *  значень.
 * Використовується метод Object.values() для отримання всіх значень об'єкта, а потім метод 
 * reduce() для обчислення суми значень.
 *
...
function getObjectValuesSum(obj) {
  if(typeof obj !== 'object') {
    ...
    let values = Object.values(obj);
    let sum = values.reduce((acc, item) => acc + item);
    return sum;
  }
...
/**
 * Функція convertArrayToObj приймає масив масивів, де кожний підмасив має два елементи 
 * - ключ та значення.
